...
for site in site_names:
    ...
    successful_launches = site_data[site_data['class'] == 1].shape[0]
    failure_launches = site_data[site_data['class'] == 0].shape[0]
...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        fig = px.pie(spacex_df, values=success_rates, 
        names=site_names, 
        title='Success rates for all of em')
        return fig
    else:
        filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site]
        success_count = filtered_df['class'].sum()
        failure_count = filtered_df['class'].count() - success_count
        success_rate = success_count / filtered_df['class'].count()
        failure_rate = 1 - success_rate
        fig = px.pie(values=[success_rate, failure_rate], 
